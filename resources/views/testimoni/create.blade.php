...
@section('breadcrumb')
    {{Breadcrumbs::render('testimoni.create')}}
@endsection
            ...
            <div class="card-body">
                ...
                <form class="custom-validation" action="{{ $actionUrl }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="responden" value="{{ old('responden') }}" required />
                        @error('responden')
                    ...
                    <div class="form-group">
                        <label>Isi</label>
                        <div>
                            <textarea required class="form-control" rows="5" name="isi">{{ old('isi') }}</textarea>
                            @error('isi')
                            <span class="text-danger">{{ $message}}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group">
                        <label>Gambar</label>
                        <input type="file" class="filestyle" data-buttonname="btn-secondary" name="gambar" value="{{ old('gambar') }}">
                        @error('gambar')
                        <span class="text-danger">{{ $message}}</span>
                        @enderror
                    </div>
